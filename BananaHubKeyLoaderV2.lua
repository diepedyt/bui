...
-- Cosmetic-only pass: clean tweaks (rounded corners, modern font, subtle border, hover).
-- Everything is pcall-wrapped so it can never error or affect the key logic.
function module:StyleUI()
	local ui = self:GetUI()
	...
	local function addHover(btn)
		pcall(function()
			...
			btn.MouseEnter:Connect(function()
				TS:Create(scale, TweenInfo.new(0.12, Enum.EasingStyle.Quad), {Scale = 1.03}):Play()
			end)
			btn.MouseLeave:Connect(function()
				TS:Create(scale, TweenInfo.new(0.12, Enum.EasingStyle.Quad), {Scale = 1}):Play()
			end)
		end)
	end

	-- ensure the panel is dead-center on screen
	pcall(function()
		ui.MainBG.AnchorPoint = Vector2.new(0.5, 0.5)
		ui.MainBG.Position = UDim2.new(0.5, 0, 0.5, 0)
	end)

	round(ui.MainBG, 14)
	round(ui.KeyUI, 12)
	round(ui.Loader, 12)
	round(ui.AutoKey.Frame, 8)
	round(ui.KeyBox.TextBox, 8)
	round(ui.Buttons.Enter, 8)
	round(ui.Buttons.GetKey, 8)
	round(ui.Buttons.BuyPermKey, 8)

	-- subtle clean border on the main panel
	pcall(function()
		local s = ui.MainBG:FindFirstChildOfClass("UIStroke") or Instance.new("UIStroke")
	...
	setFont(ui.Buttons.Enter, Enum.Font.GothamBold)
	setFont(ui.Buttons.GetKey, Enum.Font.GothamBold)
	setFont(ui.Buttons.BuyPermKey, Enum.Font.GothamBold)
	setFont(ui.KeyBox.TextBox, Enum.Font.GothamMedium)
	setFont(ui.AutoKey.Text, Enum.Font.GothamMedium)

	-- gentle hover feedback on the buttons
	addHover(ui.Buttons.Enter)
	addHover(ui.Buttons.GetKey)
	addHover(ui.Buttons.BuyPermKey)
end
...
function module:LoadUI()
	...
	local ui = self:GetUI()
	self:ProtectUI()
	pcall(function() self:StyleUI() end)

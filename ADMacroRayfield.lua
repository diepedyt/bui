...
local Rayfield =  game:GetObjects("rbxassetid://17746536343")[1]
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	local Window = {}
	function Window:CreateTab(Name,Image,Seperator)
		local SDone = false
		local TabButton = TabList.Template:Clone()
		TabButton.Seperator.Visible = (Seperator ~= nil)
		TabButton.Name = Name
...
function Tab:CreateMacroExplorer(rigs, unitData, callbacks)
			...
			local macro_navigator = Elements.Template.Macro_Navigator:Clone()
			
			
			function MacroSettings:SetNavigatorV(current_page, max_pages)
				macro_navigator.Page.Title.Text = tostring(current_page).."/"..tostring(max_pages)
				macro_navigator[">"].Visible = (current_page < max_pages)
				macro_navigator["<"].Visible = (current_page > 1)
			end
			
			MacroSettings:SetNavigatorV(1,1)

			
			local oldCons = {}
			function MacroSettings:SetCurrentNavigatorList(list)
				for _, con in pairs(oldCons) do
					con:Disconnect()
				end
				...
				local list_size = math.ceil(#list / 10)
				
				local function getMacrosByPage(page, Macros)
					local macros = {}
					
					
					for i = (page-1)*10, page*10 do
						if Macros[i] then
							table.insert(macros, Macros[i])
						end
					end
				...
				local con1 = macro_navigator["<"].Interact.MouseButton1Click:Connect(function()
					if not db then
						db = true
						current_page = current_page - 1
						MacroSettings:SetNavigatorV(current_page, list_size)
						MacroSettings:SetMacros(getMacrosByPage(current_page, list))
						db = false
					end
				end)
				
				table.insert(oldCons, con1)

				local con2 = macro_navigator[">"].Interact.MouseButton1Click:Connect(function()
					if not db then
						db = true
						current_page = current_page + 1
						MacroSettings:SetNavigatorV(current_page, list_size)
				...
				table.insert(oldCons, con2)
				
				MacroSettings:SetNavigatorV(1, list_size)
				MacroSettings:SetMacros(getMacrosByPage(1, list))
				
			end
			
			macro_navigator.Parent = oldTabPage
			macro_navigator.Visible = true
